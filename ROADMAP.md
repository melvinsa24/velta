...
| 2026-09-01 | Refactor format | **`src/lib/format.ts`** : `parseAmount` (virgule FR tolérée) et le formatage des montants, jusque-là recopiés dans 13 fichiers, deviennent une source unique. Deux rendus nommés, volontairement distincts : `formatEuros` (0 à 2 décimales — affichage courant Dashboard / Budget / Historique, un montant rond reste rond) et `formatEurosCents` (toujours 2 décimales — module Flore et CTA de saisie, sommes au centime près) ; les appels de Flore + Historique>Revenus ont été renommés en conséquence. Les deux variantes de `parseAmount` qui coexistaient (avec / sans garde `!value.trim()`) étaient équivalentes (`Number('')` vaut 0) : une seule subsiste. `formatPct` (Flore) et `round2` (saisie rapide) restent locaux. Aucun changement de comportement. `tsc --noEmit` + `eslint` + `next build` OK. |
| 2026-09-01 | Bugfix seuil de dépassement | **« ⚠ Dépassé de 0 € » à 100 % du budget**. Symptôme : une ligne de « Dépenses du mois » soldée pile au prévu s'affichait en bordure `--down` avec un message de dépassement à 0 €. **Cause** : `real > planned` comparait en égalité stricte deux flottants qui ne le sont pas — le prévu d'une dépense au prorata est un snapshot `total × ratioMelvin` à 7+ décimales (ex. 57,919621…), alors que la saisie rapide pré-remplit ce restant **arrondi au centime** (57,92). Enregistré tel quel, le réel dépasse le prévu de ~4e-4 €, que `formatEuros` arrondit à « 0 € ». Même bug **miroir** sur les soldes Flore, où le résidu empêchait au contraire de reconnaître un solde soldé (`soldeRestant === 0`, `remaining <= 0` → « Flore me doit 0,00 € » / « Reste à recevoir 0,00 € » au lieu de « Solde apuré »). **Fix** : `lib/calculs.ts` devient la source unique des comparaisons de montants — `CENT_EPSILON` (0,01) + `isCleared(remaining)` pour les soldes, et `OVERSPEND_THRESHOLD` (1 €) + `isOverspent(real, planned)` pour l'alerte. **Décision produit** : l'alerte de dépassement ne part qu'à partir d'**1 € de débordement** (en dessous c'est du bruit sur une ligne de budget) ; les soldes Flore gardent la tolérance stricte du centime — 0,80 € restant dû reste une dette, pas un solde apuré. 4 sites d'appel : `ExpenseLineCard` (alerte + bordure), `flore/page.tsx` (héro `apure`), `FlorePayments` (récap `settled`), `ExpenseQuickEntrySheet` (« Restant » en rouge, aligné sur la ligne qui ouvre la sheet — jamais l'une sans l'autre). Hors périmètre après revue, aucun n'ayant le défaut : seuil `--warn` à 80 % (franchissement, pas une égalité), jauges `RatioGauge` / `RecapRow` (écart à un objectif, tolérance ±5/15 points), `TargetsSection` (déjà arrondi au centième avant comparaison à 100). `tsc --noEmit` + `eslint` + `next build` OK. |
