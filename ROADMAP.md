...
|---|---|---|---|
| 2026-06-09 | Pré-Phase 7 | **Cohérence vocabulaire UI + SPECS** (suite de l'évolution du modèle). (1) **SPECS §6.3 réaligné** sur le nouveau modèle structurel : `parent_type` > catégorie > **dépenses nommées** (label + montant éditables inline, « + Ajouter une dépense » par catégorie, « + Nouvelle catégorie » par section, reprise N-1 = label + montant + catégorie). (2) **Renommage UI** : décision produit (validée avec Melvin) → **« Catégorie » = entrées `categories` (Réglages)**, **« Dépense » = lignes `monthly_budgets.label` (budget)**, le champ **« Type » du formulaire reste « Type »** (une catégorie ne porte pas une « catégorie » → la règle initiale « Type→Catégorie » ne s'applique pas, sinon collision avec le niveau dépense). Conséquence : le vocabulaire visible était **déjà conforme** depuis la refonte structurelle précédente (lignes budget = « dépense », sous-groupes = « catégorie »). Seul correctif code : placeholder de `CategoryForm` « Courses, Loyer, PEA » → « Alimentation, Logement, PEA » (Courses/Loyer sont désormais des *dépenses*, pas des catégories). `npm run lint` ✅, `npm run build` ✅. **Non commité.** | **Aucun changement DB ni logique métier** (session de renommage). **`SPECS §6.1` (Dashboard, Phase 8)** et **§6.2** parlent encore de « liste des catégories » avec budget/réel : non touchés car le passage éventuel à un affichage par dépense est une décision de structure Phase 8, hors périmètre d'un simple renommage — à trancher en Phase 8. **Champ « Type » conservé** dans le formulaire catégorie : si Melvin veut finalement le rebaptiser « Catégorie », il faudrait l'autre mapping (catégorie=bucket, dépense=entrée Réglages) qui imposerait de revoir la structure de l'écran Budget. **Test visuel mobile 390px** toujours à faire côté Melvin. |
...
| 2026-06-09 | Pré-Phase 7 | **Évolution du modèle de données** (décision produit) : le budget prévisionnel stocke désormais une ligne par **dépense nommée** (et non plus une ligne par catégorie). Migration `002_budget_label.sql` : `monthly_budgets.label` (text), **DROP de la contrainte unique `(month, category_id)`** (validé avec Melvin — indispensable pour que plusieurs dépenses partagent une catégorie le même mois), `transactions.monthly_budget_id` (uuid nullable, FK → `monthly_budgets` `on delete set null`) + index. SPECS §5 mis à jour (`monthly_budgets` : champ `label` + « une ligne = une dépense nommée » ; `transactions` : champ `monthly_budget_id`). Types TS (`label`, `monthly_budget_id`). **Refonte `BudgetEditor`** : hiérarchie **parent_type > catégorie > dépenses nommées** (validé avec Melvin) ; label **et** montant éditables inline (debounce 500 ms + flush au blur, timers séparés label/montant) ; « + Ajouter une dépense » sous chaque catégorie (insert ligne label vide) ; suppression de ligne (icône corbeille) ; « + Nouvelle catégorie » par section (modale CategoryForm conservée) ; « Reprendre le mois précédent » recopie désormais toutes les dépenses (label + montant + catégorie). Actions budget repensées par **id de ligne** (`createBudgetLine`/`updateBudgetLine`/`deleteBudgetLine`/`copyPreviousMonth`) au lieu d'upsert sur `(month, category_id)`. `npm run lint` ✅, `npm run build` ✅. **Non commité.** | **Migration 002 à exécuter manuellement** dans le SQL Editor Supabase (clé anon = pas de DDL). **`canCopyPrevious`** redéfini : N-1 a des dépenses ET le mois courant est vide (la copie n'a pas de dédoublonnage sans contrainte d'unicité). **SPECS §6.3** décrit encore l'ancien modèle (1 ligne = 1 catégorie) — non touché car hors périmètre demandé (édition limitée au §5) ; à réaligner. **Modale « + » de saisie transaction non touchée** (Phase 7). **Test visuel mobile 390px non réalisé** (routes auth-gated, pas de navigateur headless) → vérif build/lint/types uniquement. |
